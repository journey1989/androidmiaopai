...
def stars_app():
    ...
    log.debug('============启动app==============')
    agree = poco('com.yixia.videoeditor:id/btn_agree')
    agrees = poco.wait_for_any([agree], timeout=1800)
    agrees.click()
    if poco(text='允许').exists():
        ...
        if poco(text='始终允许').exists():
            poco(text='始终允许').click()
    quit = poco('com.yixia.videoeditor:id/tv_quit_app')
    quits = poco.wait_for_any([quit], timeout=1800)
    quits.click()
